...
def main(args):
    ...
    parser.add_argument('-d', dest='function', action='store_const', const='dir_word', help='count word occurrences in specified directory')
    ...
    if args.function == 'letter':
        ...
        for path in args.paths:
            yield from word(path, args.limit)
            ...
            for path in args.paths:
                ...
                for r in result:
                    ...
                    if os.path.isdir(temp_path):
                        continue
                    yield from word(temp_path, args.limit)
        else:
            for path in args.paths:
                for (p, _, l) in os.walk(path):
                    for i in l:
                        yield from word(os.path.join(p, i), args.limit)
